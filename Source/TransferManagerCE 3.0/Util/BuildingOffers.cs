...
namespace TransferManagerCE
{
    public class BuildingOffers
    {
        ...
        public List<OfferData> GetOffersForBuilding(ushort buildingId, List<ushort> subBuildingIds)
        {
                ...
                for (int material = 0; material < MatchStats.iMATERIAL_TOTAL_LOCATION; material++)
                {
                    ...
                    for (int priority = 7; priority >= 0; --priority)
                    {
                        ...
                        for (int offerIndex = 0; offerIndex < m_outgoingCount[offer_offset]; offerIndex++)
                        {
                            ...
                            else if (subBuildingIds.Count > 0)
                            {
                                foreach (ushort subBuildingId in subBuildingIds)
                                {
                                    if (InstanceHelper.GetBuildings(offer.m_object).Contains(subBuildingId))
                                    {
                                        offers.Add(new OfferData(reason, false, offer));
                                        break;
                                    }
